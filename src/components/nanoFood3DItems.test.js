...
import nanoFood3DItems from './nanoFood3DItems.test.js'
import nanoFood3D from './nanoFood3D'

configure({ adapter: new Adapter() });

describe('<nanoFood3DItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<nanoFood3D />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <nanoFood3DItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<nanoFood3D />);
        expect(()=> wrapper.toBeInstanceOf(nanoFood3D));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <nanoFood3DItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<nanoFood3D />);
        expect(wrapper.find(nanoFood3D)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
   
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <nanoFood3DItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<nanoFood3D />);
        expect(wrapper.find(nanoFood3D)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <nanoFood3DItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<nanoFood3D />);
        expect(wrapper.find(nanoFood3D)).toMatchObject(nanoFood3DItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. nanoFood3D does not pass properties to any child components*/
        // it('Plastic Pollution: <nanoFood3DItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(nanoFood3D).toHaveLength(1));
        // });
