...
import ActivistFooterItems from './ActivistFooterItems.test.js'
import ActivistFoot from './ActivistFooter'

configure({ adapter: new Adapter() });

describe('<ActivistFooterItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<ActivistFoot />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <ActivistFootItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<ActivistFoot />);
        expect(()=> wrapper.toBeInstanceOf(ActivistFoot));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ActivistFootItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<ActivistFoot />);
        expect(wrapper.find(ActivistFoot)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <ActivistFootItems />:Class object has existing length', () => {
        wrapper = shallow(<ActivistFoot />);
        expect(wrapper.find(ActivistFoot)).toHaveLength(ActivistFoot.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ActivistFootItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<ActivistFoot />);
        expect(wrapper.find(ActivistFoot)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <ActivistFootItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<ActivistFoot />);
        expect(wrapper.find(ActivistFoot)).toMatchObject(ActivistFooterItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. ActivistFoot does not pass properties to any child components*/
        // it('Plastic Pollution: <ActivistFootItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(ActivistFoot).toHaveLength(1));
        // });
