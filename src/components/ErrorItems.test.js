...
import ErrorItems from './ErrorItems.test.js'
...
describe('<ErrorItems />', () => {
    ...
    it('Plastic Pollution: <ErrorItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Error />);
        expect(()=> wrapper.toBeInstanceOf(Error));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ErrorItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Error />);
        expect(wrapper.find(Error)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <ErrorItems />:Class object has existing length', () => {
        wrapper = shallow(<Error />);
        expect(wrapper.find(Error)).toHaveLength(Error.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ErrorItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Error />);
    ...
    it('Plastic Pollution: <ErrorItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Error />);
        expect(wrapper.find(Error)).toMatchObject(ErrorItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Error does not pass properties to any child components*/
        // it('Plastic Pollution: <ErrorItems /> must render length of object if not authenticated!', () => {
