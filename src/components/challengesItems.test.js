...
import challengesItems from './challengesItems.test.js'
import challenges from './challenges'

configure({ adapter: new Adapter() });

describe('<challengesItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<challenges />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <challengesItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<challenges />);
        expect(()=> wrapper.toBeInstanceOf(challenges));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <challengesItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<challenges />);
        expect(wrapper.find(challenges)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <challengesItems />:Class object has existing length', () => {
        wrapper = shallow(<challenges />);
        expect(wrapper.find(challenges)).toHaveLength(challenges.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <challengesItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<challenges />);
        expect(wrapper.find(challenges)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <challengesItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<challenges />);
        expect(wrapper.find(challenges)).toMatchObject(challengesItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. challenges does not pass properties to any child components*/
        // it('Plastic Pollution: <challengesItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(challenges).toHaveLength(1));
        // });
