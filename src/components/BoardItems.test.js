...
import BoardItems from './BoardItems.test.js'
...
describe('<BoardItems />', () => {
    ...
    it('Plastic Pollution: <BoardItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Board />);
        expect(()=> wrapper.toBeInstanceOf(Board));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <BoardItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Board />);
        expect(wrapper.find(Board)).toBeDefined();
    });
  
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <BoardItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Board />);
    ...
    it('Plastic Pollution: <BoardItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Board />);
        expect(wrapper.find(Board)).toMatchObject(BoardItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Board does not pass properties to any child components*/
        // it('Plastic Pollution: <BoardItems /> must render length of object if not authenticated!', () => {
