...
describe('<landfillMap />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<landfillMap />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <landfillMap />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<landfillMap />);
        expect(()=> wrapper.toBeInstanceOf(landfillMap));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <landfillMap />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<landfillMap />);
        expect(wrapper.find(landfillMap)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <landfillMap />:Class object has existing length', () => {
        wrapper = shallow(<landfillMap />);
        expect(wrapper.find(landfillMap)).toHaveLength(landfillMap.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <landfillMap />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<landfillMap />);
        expect(wrapper.find(landfillMap)).toBeTruthy();
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. landfillMap does not pass properties to any child components*/
        // it('Plastic Pollution: <landfillMap /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(landfillMap).toHaveLength(1));
        // });
