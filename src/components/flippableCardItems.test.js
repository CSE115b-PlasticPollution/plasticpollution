...
import flippableCardItems from './flippableCardItems.test.js'
import flippableCard from './flippableCard'

configure({ adapter: new Adapter() });

describe('<flippableCardItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<flippableCard />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <flippableCardItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<flippableCard />);
        expect(()=> wrapper.toBeInstanceOf(flippableCard));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <flippableCardItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<flippableCard />);
        expect(wrapper.find(flippableCard)).toBeDefined();
    });
    
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <flippableCardItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<flippableCard />);
        expect(wrapper.find(flippableCard)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <flippableCardItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<flippableCard />);
        expect(wrapper.find(flippableCard)).toMatchObject(flippableCardItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. flippableCard does not pass properties to any child components*/
        // it('Plastic Pollution: <flippableCardItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(flippableCard).toHaveLength(1));
        // });
