...
import ActivistVideoItems from './ActivistVideoItems.test.js'
import ActivistVideo from './ActivistVideo'

configure({ adapter: new Adapter() });

describe('<ActivistVideoItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<ActivistVideo />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <ActivistVideoItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<ActivistVideo />);
        expect(()=> wrapper.toBeInstanceOf(ActivistVideo));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ActivistVideoItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<ActivistVideo />);
        expect(wrapper.find(ActivistVideo)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <ActivistVideoItems />:Class object has existing length', () => {
        wrapper = shallow(<ActivistVideo />);
        expect(wrapper.find(ActivistVideo)).toHaveLength(ActivistVideo.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ActivistVideoItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<ActivistVideo />);
        expect(wrapper.find(ActivistVideo)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <ActivistVideoItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<ActivistVideo />);
        expect(wrapper.find(ActivistVideo)).toMatchObject(ActivistVideoItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. ActivistVideo does not pass properties to any child components*/
        // it('Plastic Pollution: <ActivistVideoItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(ActivistVideo).toHaveLength(1));
        // });
