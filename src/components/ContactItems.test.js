...
import ContactItems from './ContactItems.test.js'
...
describe('<ContactItems />', () => {
    ...
    it('Plastic Pollution: <ContactItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Contact />);
        expect(()=> wrapper.toBeInstanceOf(Contact));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ContactItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Contact />);
        expect(wrapper.find(Contact)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <ContactItems />:Class object has existing length', () => {
        wrapper = shallow(<Contact />);
        expect(wrapper.find(Contact)).toHaveLength(Contact.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ContactItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Contact />);
    ...
    it('Plastic Pollution: <ContactItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Contact />);
        expect(wrapper.find(Contact)).toMatchObject(ContactItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Contact does not pass properties to any child components*/
        // it('Plastic Pollution: <ContactItems /> must render length of object if not authenticated!', () => {
