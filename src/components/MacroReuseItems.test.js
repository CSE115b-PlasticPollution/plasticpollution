...
import MacroReuseItems from './MacroReuseItems.test.js'
import MacroReuse from './MacroReuse'

configure({ adapter: new Adapter() });

describe('<MacroReuseItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<MacroReuse />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <MacroReuseItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<MacroReuse />);
        expect(()=> wrapper.toBeInstanceOf(MacroReuse));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <MacroReuseItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<MacroReuse />);
        expect(wrapper.find(MacroReuse)).toBeDefined();
    });
  
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <MacroReuseItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<MacroReuse />);
        expect(wrapper.find(MacroReuse)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <MacroReuseItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<MacroReuse />);
        expect(wrapper.find(MacroReuse)).toMatchObject(MacroReuseItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. MacroReuse does not pass properties to any child components*/
        // it('Plastic Pollution: <MacroReuseItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(MacroReuse).toHaveLength(1));
        // });
