...
import BoardCheckItems from './BoardCheckItems.test.js'
import BoardCheck from './BoardCheck'

configure({ adapter: new Adapter() });

describe('<BoardCheckItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<BoardCheck />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <BoardCheckItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<BoardCheck />);
        expect(()=> wrapper.toBeInstanceOf(BoardCheck));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <BoardCheckItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<BoardCheck />);
        expect(wrapper.find(BoardCheck)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
  
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <BoardCheckItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<BoardCheck />);
        expect(wrapper.find(BoardCheck)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <BoardCheckItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<BoardCheck />);
        expect(wrapper.find(BoardCheck)).toMatchObject(BoardCheckItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. BoardCheck does not pass properties to any child components*/
        // it('Plastic Pollution: <BoardCheckItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(BoardCheck).toHaveLength(1));
        // });
