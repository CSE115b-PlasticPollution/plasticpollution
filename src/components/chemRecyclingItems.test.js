...
import chemRecyclingItems from './chemRecyclingItems.test.js'
import chemRecycling from './chemRecycling'

configure({ adapter: new Adapter() });

describe('<chemRecyclingItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<chemRecycling />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <chemRecyclingItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<chemRecycling />);
        expect(()=> wrapper.toBeInstanceOf(chemRecycling));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <chemRecyclingItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<chemRecycling />);
        expect(wrapper.find(chemRecycling)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <chemRecyclingItems />:Class object has existing length', () => {
        wrapper = shallow(<chemRecycling />);
        expect(wrapper.find(chemRecycling)).toHaveLength(chemRecycling.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <chemRecyclingItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<chemRecycling />);
        expect(wrapper.find(chemRecycling)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <chemRecyclingItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<chemRecycling />);
        expect(wrapper.find(chemRecycling)).toMatchObject(chemRecyclingItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. chemRecycling does not pass properties to any child components*/
        // it('Plastic Pollution: <chemRecyclingItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(chemRecycling).toHaveLength(1));
        // });
