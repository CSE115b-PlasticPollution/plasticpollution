...
import frozenFishItems from './frozenFishItems.test.js'
import frozenFish from './frozenFish'

configure({ adapter: new Adapter() });

describe('<frozenFishItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<frozenFish />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <frozenFishItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<frozenFish />);
        expect(()=> wrapper.toBeInstanceOf(frozenFish));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <frozenFishItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<frozenFish />);
        expect(wrapper.find(frozenFish)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <frozenFishItems />:Class object has existing length', () => {
        wrapper = shallow(<frozenFish />);
        expect(wrapper.find(frozenFish)).toHaveLength(frozenFish.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <frozenFishItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<frozenFish />);
        expect(wrapper.find(frozenFish)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <frozenFishItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<frozenFish />);
        expect(wrapper.find(frozenFish)).toMatchObject(frozenFishItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. frozenFish does not pass properties to any child components*/
        // it('Plastic Pollution: <frozenFishItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(frozenFish).toHaveLength(1));
        // });
