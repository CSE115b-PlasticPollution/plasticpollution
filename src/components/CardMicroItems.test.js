...
import CardMicroItems from './CardMicroItems.test.js'
import CardMicro from './CardMicro'

configure({ adapter: new Adapter() });

describe('<CardMicroItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<CardMicro />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <CardMicroItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<CardMicro />);
        expect(()=> wrapper.toBeInstanceOf(CardMicro));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <CardMicroItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<CardMicro />);
        expect(wrapper.find(CardMicro)).toBeDefined();
    });

    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <CardMicroItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<CardMicro />);
        expect(wrapper.find(CardMicro)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <CardMicroItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<CardMicro />);
        expect(wrapper.find(CardMicro)).toMatchObject(CardMicroItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. CardMicro does not pass properties to any child components*/
        // it('Plastic Pollution: <CardMicroItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(CardMicro).toHaveLength(1));
        // });
