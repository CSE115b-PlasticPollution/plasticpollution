...
import PreventionJourneyItems from './PreventionJourneyItems.test.js'
import PreventionJourney from './PreventionJourney'

configure({ adapter: new Adapter() });

describe('<PreventionJourneyItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<PreventionJourney />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <PreventionJourneyItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<PreventionJourney />);
        expect(()=> wrapper.toBeInstanceOf(PreventionJourney));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <PreventionJourneyItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<PreventionJourney />);
        expect(wrapper.find(PreventionJourney)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <PreventionJourneyItems />:Class object has existing length', () => {
        wrapper = shallow(<PreventionJourney />);
        expect(wrapper.find(PreventionJourney)).toHaveLength(PreventionJourney.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <PreventionJourneyItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<PreventionJourney />);
        expect(wrapper.find(PreventionJourney)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <PreventionJourneyItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<PreventionJourney />);
        expect(wrapper.find(PreventionJourney)).toMatchObject(PreventionJourneyItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. PreventionJourney does not pass properties to any child components*/
        // it('Plastic Pollution: <PreventionJourneyItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(PreventionJourney).toHaveLength(1));
        // });
