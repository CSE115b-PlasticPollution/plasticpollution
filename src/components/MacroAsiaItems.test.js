...
import MacroAsiaItems from './MacroAsiaItems.test.js'
import MacroAsia from './MacroAsia'

configure({ adapter: new Adapter() });

describe('<MacroAsiaItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<MacroAsia />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <MacroAsiaItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<MacroAsia />);
        expect(()=> wrapper.toBeInstanceOf(MacroAsia));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <MacroAsiaItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<MacroAsia />);
        expect(wrapper.find(MacroAsia)).toBeDefined();
    });
 
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <MacroAsiaItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<MacroAsia />);
        expect(wrapper.find(MacroAsia)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <MacroAsiaItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<MacroAsia />);
        expect(wrapper.find(MacroAsia)).toMatchObject(MacroAsiaItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. MacroAsia does not pass properties to any child components*/
        // it('Plastic Pollution: <MacroAsiaItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(MacroAsia).toHaveLength(1));
        // });
