...
import MacroDumpItems from './MacroDumpItems.test.js'
import MacroDump from './MacroDump'

configure({ adapter: new Adapter() });

describe('<MacroDumpItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<MacroDump />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <MacroDumpItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<MacroDump />);
        expect(()=> wrapper.toBeInstanceOf(MacroDump));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <MacroDumpItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<MacroDump />);
        expect(wrapper.find(MacroDump)).toBeDefined();
    });
  
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <MacroDumpItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<MacroDump />);
        expect(wrapper.find(MacroDump)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <MacroDumpItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<MacroDump />);
        expect(wrapper.find(MacroDump)).toMatchObject(MacroDumpItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. MacroDump does not pass properties to any child components*/
        // it('Plastic Pollution: <MacroDumpItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(MacroDump).toHaveLength(1));
        // });
