...
import popOverNanoItems from './popOverNanoItems.test.js'
import popOverNano from './popOverNano'

configure({ adapter: new Adapter() });

describe('<popOverNanoItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<popOverNano />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <popOverNanoItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<popOverNano />);
        expect(()=> wrapper.toBeInstanceOf(popOverNano));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <popOverNanoItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<popOverNano />);
        expect(wrapper.find(popOverNano)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <popOverNanoItems />:Class object has existing length', () => {
        wrapper = shallow(<popOverNano />);
        expect(wrapper.find(popOverNano)).toHaveLength(popOverNano.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <popOverNanoItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<popOverNano />);
        expect(wrapper.find(popOverNano)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <popOverNanoItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<popOverNano />);
        expect(wrapper.find(popOverNano)).toMatchObject(popOverNanoItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. popOverNano does not pass properties to any child components*/
        // it('Plastic Pollution: <popOverNanoItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(popOverNano).toHaveLength(1));
        // });
