...
import ActivistCarouselItems from './ActivistCarouselItems.test.js'
import ActivistCarousel from './ActivistCarousel'

configure({ adapter: new Adapter() });

describe('<ActivistCarouselItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<ActivistCarousel />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <ActivistCarouselItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<ActivistCarousel />);
        expect(()=> wrapper.toBeInstanceOf(ActivistCarousel));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ActivistCarouselItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<ActivistCarousel />);
        expect(wrapper.find(ActivistCarousel)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <ActivistCarouselItems />:Class object has existing length', () => {
        wrapper = shallow(<ActivistCarousel />);
        expect(wrapper.find(ActivistCarousel)).toHaveLength(ActivistCarousel.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ActivistCarouselItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<ActivistCarousel />);
        expect(wrapper.find(ActivistCarousel)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <ActivistCarouselItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<ActivistCarousel />);
        expect(wrapper.find(ActivistCarousel)).toMatchObject(ActivistCarouselItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. ActivistCarousel does not pass properties to any child components*/
        // it('Plastic Pollution: <ActivistCarouselItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(ActivistCarousel).toHaveLength(1));
        // });
