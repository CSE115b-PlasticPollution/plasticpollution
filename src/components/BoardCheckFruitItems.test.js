...
import BoardCheckFruitItems from './BoardCheckFruitItems.test.js'
import BoardCheckFruit from './BoardCheckFruit'

configure({ adapter: new Adapter() });

describe('<BoardCheckFruitItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<BoardCheckFruit />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <BoardCheckFruitItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<BoardCheckFruit />);
        expect(()=> wrapper.toBeInstanceOf(BoardCheckFruit));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <BoardCheckFruitItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<BoardCheckFruit />);
        expect(wrapper.find(BoardCheckFruit)).toBeDefined();
    });
    
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <BoardCheckFruitItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<BoardCheckFruit />);
        expect(wrapper.find(BoardCheckFruit)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <BoardCheckFruitItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<BoardCheckFruit />);
        expect(wrapper.find(BoardCheckFruit)).toMatchObject(BoardCheckFruitItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. BoardCheckFruit does not pass properties to any child components*/
        // it('Plastic Pollution: <BoardCheckFruitItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(BoardCheckFruit).toHaveLength(1));
        // });
