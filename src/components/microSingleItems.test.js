...
import microSingleItems from './microSingleItems.test.js'
import microSingle from './microSingle'

configure({ adapter: new Adapter() });

describe('<microSingleItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<microSingle />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <microSingleItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<microSingle />);
        expect(()=> wrapper.toBeInstanceOf(microSingle));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <microSingleItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<microSingle />);
        expect(wrapper.find(microSingle)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <microSingleItems />:Class object has existing length', () => {
        wrapper = shallow(<microSingle />);
        expect(wrapper.find(microSingle)).toHaveLength(microSingle.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <microSingleItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<microSingle />);
        expect(wrapper.find(microSingle)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <microSingleItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<microSingle />);
        expect(wrapper.find(microSingle)).toMatchObject(microSingleItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. microSingle does not pass properties to any child components*/
        // it('Plastic Pollution: <microSingleItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(microSingle).toHaveLength(1));
        // });
