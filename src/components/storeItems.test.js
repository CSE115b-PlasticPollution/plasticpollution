...
import storeItems from './storeItems.test.js'
...
describe('<storeItems />', () => {
    ...
    it('Plastic Pollution: <storeItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<store />);
        expect(()=> wrapper.toBeInstanceOf(store));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <storeItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<store />);
        expect(wrapper.find(store)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <storeItems />:Class object has existing length', () => {
        wrapper = shallow(<store />);
        expect(wrapper.find(store)).toHaveLength(store.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <storeItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<store />);
        expect(wrapper.find(store)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <storeItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<store />);
        expect(wrapper.find(store)).toMatchObject(storeItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. store does not pass properties to any child components*/
        // it('Plastic Pollution: <storeItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(store).toHaveLength(1));
        // });
