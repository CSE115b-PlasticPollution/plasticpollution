...
import ActivistJumboItems from './ActivistJumboItems.test.js'
import ActivistJumbo from './ActivistJumbo'

configure({ adapter: new Adapter() });

describe('<ActivistJumboItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<ActivistJumbo />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <ActivistJumboItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<ActivistJumbo />);
        expect(()=> wrapper.toBeInstanceOf(ActivistJumbo));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ActivistJumboItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<ActivistJumbo />);
        expect(wrapper.find(ActivistJumbo)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <ActivistJumboItems />:Class object has existing length', () => {
        wrapper = shallow(<ActivistJumbo />);
        expect(wrapper.find(ActivistJumbo)).toHaveLength(ActivistJumbo.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ActivistJumboItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<ActivistJumbo />);
        expect(wrapper.find(ActivistJumbo)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <ActivistJumboItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<ActivistJumbo />);
        expect(wrapper.find(ActivistJumbo)).toMatchObject(ActivistJumboItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. ActivistJumbo does not pass properties to any child components*/
        // it('Plastic Pollution: <ActivistJumboItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(ActivistJumbo).toHaveLength(1));
        // });
