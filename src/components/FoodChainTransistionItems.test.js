...
import FoodChainTransistionItems from './FoodChainTransistionItems.test.js'
import FoodChainTransistion from './FoodChainTransistion'

configure({ adapter: new Adapter() });

describe('<FoodChainTransistionItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<FoodChainTransistion />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <FoodChainTransistionItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<FoodChainTransistion />);
        expect(()=> wrapper.toBeInstanceOf(FoodChainTransistion));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <FoodChainTransistionItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<FoodChainTransistion />);
        expect(wrapper.find(FoodChainTransistion)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <FoodChainTransistionItems />:Class object has existing length', () => {
        wrapper = shallow(<FoodChainTransistion />);
        expect(wrapper.find(FoodChainTransistion)).toHaveLength(FoodChainTransistion.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <FoodChainTransistionItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<FoodChainTransistion />);
        expect(wrapper.find(FoodChainTransistion)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <FoodChainTransistionItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<FoodChainTransistion />);
        expect(wrapper.find(FoodChainTransistion)).toMatchObject(FoodChainTransistionItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. FoodChainTransistion does not pass properties to any child components*/
        // it('Plastic Pollution: <FoodChainTransistionItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(FoodChainTransistion).toHaveLength(1));
        // });
