...
import AdventureItems from './AdventureItems.test.js'
...
describe('<AdventureItems />', () => {
    ...
    beforeEach(()=> {
        wrapper = shallow(<Adventure />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <AdventureItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Adventure />);
        expect(()=> wrapper.toBeInstanceOf(Adventure));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <AdventureItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Adventure />);
        expect(wrapper.find(Adventure)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <AdventureItems />:Class object has existing length', () => {
        wrapper = shallow(<Adventure />);
        expect(wrapper.find(Adventure)).toHaveLength(Adventure.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <AdventureItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Adventure />);
        expect(wrapper.find(Adventure)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <AdventureItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Adventure />);
        expect(wrapper.find(Adventure)).toMatchObject(AdventureItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Adventure does not pass properties to any child components*/
        // it('Plastic Pollution: <AdventureItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(Adventure).toHaveLength(1));
        // });
