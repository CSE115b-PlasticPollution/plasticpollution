...
import MacroItems from './MacroItems.test.js'
...
describe('<MacroItems />', () => {
    ...
    beforeEach(()=> {
        wrapper = shallow(<Macro />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <MacroItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Macro />);
        expect(()=> wrapper.toBeInstanceOf(Macro));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <MacroItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Macro />);
        expect(wrapper.find(Macro)).toBeDefined();
    });
 
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <MacroItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Macro />);
        expect(wrapper.find(Macro)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <MacroItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Macro />);
        expect(wrapper.find(Macro)).toMatchObject(MacroItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Macro does not pass properties to any child components*/
        // it('Plastic Pollution: <MacroItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(Macro).toHaveLength(1));
        // });
