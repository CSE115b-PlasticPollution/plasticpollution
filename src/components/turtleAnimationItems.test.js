...
import turtleAnimationItems from './turtleAnimationItems.test.js'
import turtleAnimation from './turtleAnimation'

configure({ adapter: new Adapter() });

describe('<turtleAnimationItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<turtleAnimation />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <turtleAnimationItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<turtleAnimation />);
        expect(()=> wrapper.toBeInstanceOf(turtleAnimation));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <turtleAnimationItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<turtleAnimation />);
        expect(wrapper.find(turtleAnimation)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <turtleAnimationItems />:Class object has existing length', () => {
        wrapper = shallow(<turtleAnimation />);
        expect(wrapper.find(turtleAnimation)).toHaveLength(turtleAnimation.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <turtleAnimationItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<turtleAnimation />);
        expect(wrapper.find(turtleAnimation)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <turtleAnimationItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<turtleAnimation />);
        expect(wrapper.find(turtleAnimation)).toMatchObject(turtleAnimationItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. turtleAnimation does not pass properties to any child components*/
        // it('Plastic Pollution: <turtleAnimationItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(turtleAnimation).toHaveLength(1));
        // });
