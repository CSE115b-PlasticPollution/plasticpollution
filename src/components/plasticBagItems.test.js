...
import plasticBagItems from './plasticBagItems.test.js'
import plasticBag from './plasticBag'

configure({ adapter: new Adapter() });

describe('<plasticBagItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<plasticBag />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <plasticBagItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<plasticBag />);
        expect(()=> wrapper.toBeInstanceOf(plasticBag));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <plasticBagItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<plasticBag />);
        expect(wrapper.find(plasticBag)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <plasticBagItems />:Class object has existing length', () => {
        wrapper = shallow(<plasticBag />);
        expect(wrapper.find(plasticBag)).toHaveLength(plasticBag.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <plasticBagItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<plasticBag />);
        expect(wrapper.find(plasticBag)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <plasticBagItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<plasticBag />);
        expect(wrapper.find(plasticBag)).toMatchObject(plasticBagItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. plasticBag does not pass properties to any child components*/
        // it('Plastic Pollution: <plasticBagItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(plasticBag).toHaveLength(1));
        // });
