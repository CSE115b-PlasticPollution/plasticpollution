...
import FaqItems from './FaqItems.test.js'
import Faq from './Faq'
...
describe('<FaqItems />', () => {
    ...
    it('Plastic Pollution: <FaqItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Faq />);
        expect(()=> wrapper.toBeInstanceOf(Faq));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <FaqItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Faq />);
        expect(wrapper.find(Faq)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <FaqItems />:Class object has existing length', () => {
        wrapper = shallow(<Faq />);
        expect(wrapper.find(Faq)).toHaveLength(Faq.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <FaqItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Faq />);
        expect(wrapper.find(Faq)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <FaqItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Faq />);
        expect(wrapper.find(Faq)).toMatchObject(FaqItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Faq does not pass properties to any child components*/
        // it('Plastic Pollution: <FaqItems /> must render length of object if not authenticated!', () => {
