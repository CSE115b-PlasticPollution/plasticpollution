...
import ActivistLinksItems from './ActivistLinksItems.test.js'
import ActivistLinks from './ActivistLinks'

configure({ adapter: new Adapter() });

describe('<ActivistLinksItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<ActivistLinks />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <ActivistLinksItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<ActivistLinks />);
        expect(()=> wrapper.toBeInstanceOf(ActivistLinks));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <ActivistLinksItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<ActivistLinks />);
        expect(wrapper.find(ActivistLinks)).toBeDefined();
    });
 
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <ActivistLinksItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<ActivistLinks />);
        expect(wrapper.find(ActivistLinks)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <ActivistLinksItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<ActivistLinks />);
        expect(wrapper.find(ActivistLinks)).toMatchObject(ActivistLinksItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. ActivistLinks does not pass properties to any child components*/
        // it('Plastic Pollution: <ActivistLinksItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(ActivistLinks).toHaveLength(1));
        // });
