...
import Macro2Items from './Macro2Items.test.js'
import Macro2 from './Macro2'

configure({ adapter: new Adapter() });

describe('<Macro2Items />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<Macro2 />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <Macro2Items />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<Macro2 />);
        expect(()=> wrapper.toBeInstanceOf(Macro2));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <Macro2Items />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<Macro2 />);
        expect(wrapper.find(Macro2)).toBeDefined();
    });
 
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <Macro2Items />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<Macro2 />);
        expect(wrapper.find(Macro2)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <Macro2Items />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<Macro2 />);
        expect(wrapper.find(Macro2)).toMatchObject(Macro2Items);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. Macro2 does not pass properties to any child components*/
        // it('Plastic Pollution: <Macro2Items /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(Macro2).toHaveLength(1));
        // });
