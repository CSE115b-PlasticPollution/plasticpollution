...
import washingmachineItems from './washingmachineItems.test.js'
import washingmachine from './washingmachine'

configure({ adapter: new Adapter() });

describe('<washingmachineItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<washingmachine />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <washingmachineItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<washingmachine />);
        expect(()=> wrapper.toBeInstanceOf(washingmachine));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <washingmachineItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<washingmachine />);
        expect(wrapper.find(washingmachine)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <washingmachineItems />:Class object has existing length', () => {
        wrapper = shallow(<washingmachine />);
        expect(wrapper.find(washingmachine)).toHaveLength(washingmachine.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <washingmachineItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<washingmachine />);
        expect(wrapper.find(washingmachine)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <washingmachineItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<washingmachine />);
        expect(wrapper.find(washingmachine)).toMatchObject(washingmachineItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. washingmachine does not pass properties to any child components*/
        // it('Plastic Pollution: <washingmachineItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(washingmachine).toHaveLength(1));
        // });
