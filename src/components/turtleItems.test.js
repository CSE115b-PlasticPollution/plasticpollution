...
import turtleItems from './turtleItems.test.js'
import turtle from './turtle'

configure({ adapter: new Adapter() });

describe('<turtleItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<turtle />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <turtleItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<turtle />);
        expect(()=> wrapper.toBeInstanceOf(turtle));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <turtleItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<turtle />);
        expect(wrapper.find(turtle)).toBeDefined();
    });
    /* Checks length of object, must be greater then 0*/
    it('Plastic Pollution: <turtleItems />:Class object has existing length', () => {
        wrapper = shallow(<turtle />);
        expect(wrapper.find(turtle)).toHaveLength(turtle.length);
    });
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <turtleItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<turtle />);
        expect(wrapper.find(turtle)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <turtleItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<turtle />);
        expect(wrapper.find(turtle)).toMatchObject(turtleItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. turtle does not pass properties to any child components*/
        // it('Plastic Pollution: <turtleItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(turtle).toHaveLength(1));
        // });
