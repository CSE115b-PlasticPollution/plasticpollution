...
import MacroOceanItems from './MacroOceanItems.test.js'
import MacroOcean from './MacroOcean'

configure({ adapter: new Adapter() });

describe('<MacroOceanItems />', () => {
    let wrapper;

    beforeEach(()=> {
        wrapper = shallow(<MacroOcean />);
    });
    /* Use .toBeInstanceOf(Class) to check that an object is an instance of a class. This matcher uses instanceof underneath. */
    it('Plastic Pollution: <MacroOceanItems />:Object is an instance of this class; Matches user instance', () => {
        wrapper = shallow(<MacroOcean />);
        expect(()=> wrapper.toBeInstanceOf(MacroOcean));
    });
    /* Checks that a variable is not undefined, or checks function and returns true */
    it('Plastic Pollution: <MacroOceanItems />:Rendered correct defined variables and functions', () => {
        wrapper = shallow(<MacroOcean />);
        expect(wrapper.find(MacroOcean)).toBeDefined();
    });
 
    /* If we rendered an object to screen, return true*/
    it('Plastic Pollution: <MacroOceanItems />:Rendered object correctly to screen!', () => {
        wrapper = shallow(<MacroOcean />);
        expect(wrapper.find(MacroOcean)).toBeTruthy();
    });
    /* Checks if javascript object matches a subset of properties of an object, return true if true*/
    it('Plastic Pollution: <MacroOceanItems />:Rendered object matches subset properties!', () => {
        wrapper = shallow(<MacroOcean />);
        expect(wrapper.find(MacroOcean)).toMatchObject(MacroOceanItems);
    });
    ...
    it('Checking Local Server Links: Rendered correctly', () => {
        ...
        /* Only activiated if checking for properties. MacroOcean does not pass properties to any child components*/
        // it('Plastic Pollution: <MacroOceanItems /> must render length of object if not authenticated!', () => {
        //     wrapper.setProps({somePropertyFoo:true});
        //     expect(wrapper.find(MacroOcean).toHaveLength(1));
        // });
